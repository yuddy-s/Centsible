...
const app = new Hono()
    ...
    async (c) => {
        ...
        .where(
            and(
                ...
                eq(accounts.userId, auth.userId) //to see if user Id currently matches logged in user and to return account only belonging to that user
            )
        );
    ...
    .post(
        ...
        async (c) => {
            ...
            const [data] = await db.insert(transactions).values({ /*putting brackets around data destructures it and we can just return data in the json as is, use only if one element that you are returning */
                id: createId(),
                ...values,
